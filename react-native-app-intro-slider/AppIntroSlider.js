...
export default class AppIntroSlider extends React.Component {
 ...
 //here change for skip text color
 _renderDefaultButton = (name) => {
    let content = (name === 'Skip')? <Text style={{color:"#84432d",fontSize: 18,fontFamily: 'Century Gothic', padding: 16,flexDirection:"row"}}>{this.props[`${name.toLowerCase()}Label`]}</Text>:<Text style={{color:"black",fontSize: 18, padding: 16,flexDirection:"row",fontFamily: 'Century Gothic'}}>{this.props[`${name.toLowerCase()}Label`]}</Text>;
//  let content = (name === 'Skip')? <Text style={styles.buttonText}>{this.props[`${name.toLowerCase()}Label`]}</Text>:;
 if (this.props.bottomButton) {
 content = <View style={[styles.bottomButton, (name === 'Skip' || name === 'Prev') && { backgroundColor: 'transparent' }]}>{content}</View>
 }
 return content;
...
//change here 
 ...
 />
 {this._renderPagination()}
 {/* <View style={{height:3,backgroundColor:"grey",opacity:0.3}}/> */}
