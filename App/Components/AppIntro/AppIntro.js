     ...
     if(this.props.appIntroReducer.checkprefRes!=''){
      ...
      //  this.setState({isSpinnerVisible:false})
       ...
       //  console.log("Response+++++++++++++++ SCreen++++++"+JSON.stringify(this.props.appIntroReducer.checkprefRes.data.savedScreen))
         if(this.props.appIntroReducer.checkprefRes.data.savedScreen == STRINGS.SCREEN_ONE){Actions.contact({type:'reset'})  }
        else if(this.props.appIntroReducer.checkprefRes.data.savedScreen == STRINGS.SCREEN_TWO){Actions.servicePreferences({type:'reset'})  }
        else if(this.props.appIntroReducer.checkprefRes.data.savedScreen == STRINGS.SCREEN_THREE){Actions.pregnancyselection({type:'reset'})  }
        else if(this.props.appIntroReducer.checkprefRes.data.savedScreen == STRINGS.SCREEN_THREE_YES){Actions.pregnancyduedate({type:'reset'})  }
        else if(this.props.appIntroReducer.checkprefRes.data.savedScreen == STRINGS.SCREEN_THREE_NO){Actions.kidsselection({type:'reset'})  }
        else  if(this.props.appIntroReducer.checkprefRes.data.savedScreen == STRINGS.SCREEN_FOUR){Actions.whattobuy({type:'reset'})  }
       else  if(this.props.appIntroReducer.checkprefRes.data.savedScreen == STRINGS.SCREEN_SIX){Actions.childDobInformation({type:'reset'})  }
       else if(this.props.appIntroReducer.checkprefRes.data.savedScreen == STRINGS.SCREEN_SEVEN){Actions.sizeandproportion({type:'reset'})  }
       else if(this.props.appIntroReducer.checkprefRes.data.savedScreen == STRINGS.SCREEN_EIGHT){Actions.brandSelection({type:'reset'})  }
      else if(this.props.appIntroReducer.checkprefRes.data.savedScreen == STRINGS.SCREEN_NINE){Actions.stylecomponent({type:'reset'})  }
      else if(this.props.appIntroReducer.checkprefRes.data.savedScreen == STRINGS.SCREEN_TEN){Actions.talktoUs({type:'reset'})  }
      else  if(this.props.appIntroReducer.checkprefRes.data.savedScreen == STRINGS.SCREEN_ELEVEN){Actions.appIntro({type:'reset'})  }
   ...
   // <ScrollView style={{flex:1}}>
       ...
       <AppIntroSlider
        ...
        <View  style={{position:"absolute",bottom:80,left:0,right:0,justifyContent:"center",alignItems:"center"}}>
         <TouchableWithoutFeedback  onPress={() => this._gotToLoginScreen()}>
              <View style={{flexDirection:"row"}}><Text>Already have account ? </Text><Text style={{fontWeight:"bold",color:COLORS.TEXT_COLOR}}>Login</Text>  </View></TouchableWithoutFeedback>   
        </View>
