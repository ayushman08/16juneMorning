...
class ChildInformationDob extends Component {
  ...
  fillData(userData) {
    ...
    if (userData.kids.length > 0) {
      this.onChildNameChanged(userData.kids[userData.kids.length - 1].name)
      if (userData.kids[userData.kids.length - 1].gender === 1) {
        this.setState({ isBoy: true, isGirl: false, genderSelected: true })
      } else {
        this.setState({ isBoy: false, isGirl: true, genderSelected: true })
      }

      this.setState({ pickedDate: userData.kids[userData.kids.length - 1].dob })
      this.setState({ kidsDataRecieved: userData.kids })

      // if(userData.kids.length>0 && userData.kids!=''){
      //   userData.kids.map((item)=>{
      //     kids.push(item)
      //   })
  ...
  componentDidUpdate() {
    ...
    if (this.props.childInfoReducer.childInfoRes != '') {
      ...
      if (this.props.childInfoReducer.childInfoRes.status === "success") {
        ...
        AsyncStorage.setItem("UserInfo", JSON.stringify(this.props.childInfoReducer.childInfoRes.data));
...
//   componentWillReceiveProps(newProps){
//     if(newProps.fromscreen!='undefined'){
//       if(newProps.fromscreen==='style'){
//         this.props.resetState()
//     }
//   }
// }
  ...
  submit() {
    ...
    if (this.props.childInfoReducer.child_name.trim() == '') {
    ...
    else if (ageFromString > 6) { alert("Please select Correct date") }
      ...
      var kids = []
      if (this.state.kidsDataRecieved.length < this.state.userInfo.kids_enroll) {
        ...
        if (this.state.kidsDataRecieved.length > 0 && this.state.kidsDataRecieved != '') {
          for (var i = 0; i < this.state.kidsDataRecieved.length - 1; i++) {
            kids.push(this.state.kidsDataRecieved[i])
          }
        }
        console.log("Size>>" + this.state.kidsDataRecieved[this.state.kidsDataRecieved.length - 1].size);
        var data = {
          name: this.props.childInfoReducer.child_name,
          dob: this.state.pickedDate,
          gender: genderSelection,
          size: this.state.kidsDataRecieved[this.state.kidsDataRecieved.length - 1].size,
          _id: this.state.kidsDataRecieved[this.state.kidsDataRecieved.length - 1]._id,
          proportion: this.state.kidsDataRecieved[this.state.kidsDataRecieved.length - 1].proportion,
          fit_size: this.state.kidsDataRecieved[this.state.kidsDataRecieved.length - 1].fit_size,
          other_brand: this.state.kidsDataRecieved[this.state.kidsDataRecieved.length - 1].other_brand,
          brand: this.state.kidsDataRecieved[this.state.kidsDataRecieved.length - 1].brand
      ...
      postData =
        {
          ...
          savedScrren: Strings.SCREEN_SEVEN,
          kids: kids
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={{ textAlign: "center", color: 'black', fontWeight: "600", marginTop: 40, marginBottom: 20, fontSize: 18 }}>{this.state.title} </Text>:
          ...
          <View style={{ marginTop: 20, flexDirection: "row" }}>
            ...
            {/* <Text style={{ color: "grey", marginTop: 30, fontSize: 18 }}>( YYYY )</Text> */}
          </View>
